...
<!--        <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/style_2.css"/>-->
        ...
        <style>
            ...
            main .profile, main .about, main .resume, main .works, main .blog, main .schedule, main .cancel, main .paid {
            ...
            main .about, main .resume, main .works, main .blog, main .schedule, main .cancel , main .paid{
            ...
            @media only screen and (min-width: 1040px) {
                ...
                main .about, main .resume, main .schedule, main .cancel, main .paid {
                    width: 0;
                    height: 0;
                    opacity: 0;
                    overflow: hidden;
    ...
    <body>
        <div class="page">
            <header>
                <nav>
                    ...
                    <a href="#resume" id='getResume'>
                        <span class="fas fa-battery-2"></span>
            ...
            <main>
                ...
                <!--                <script>
                                    var listIdRequest = [
                <c:forEach var="item" items="${listIdRequest}">
                ...
                <section id="about" class="about view">
                        ...
                        <tbody>
                            ...
                            <c:forEach items="${requestScope.dataRP}" var="s" varStatus="loop">
                                <tr>
                                    ...
                                    <c:if test="${loop.index == 0 || !s.mentor.fullname.equals(requestScope.dataRP[loop.index-1].mentor.fullname) ||
                                          ...
                                          <td>
                                              <form action="listrequest" method="post"  class="cancel-form">
                                                  <input type="hidden" name="idRequest" value="" class="id-request-input" data-id-request="${s.idRequest}"/>
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            ...
                            forms.forEach(form => {
                                const sIdRequest = form.querySelector('.id-request-input').getAttribute('data-id-request');
                                listIdRequest.forEach(lid => {
                                    if (lid.includes('-' + sIdRequest + '-')) {
                                        form.querySelector('.id-request-input').value = lid;
                                    }
                                });
                            });
                        });
                    ...
                    <div class="pagination-container">
                        <div style="display: flex; justify-content: center; align-items: center;">
                            <c:forEach begin="${1}" end="${requestScope.numRP}" var="i">
                                <p class="dispage" style="background-color: #000;  width: 20px; height: 30px; display: block; margin-right: 5px; cursor: pointer; clear: both"><a class="${i==pageRP?'active':''}" style="color: white; padding-left: 0px; justify-content: center;align-items: center;" href="listrequest?pageRP=${i}">${i}</a></p>                                
                                </c:forEach>
                        </div>
                    </div>
                </section>
                    ...
                    <div class="pagination-container">
                        <div style="display: flex; justify-content: center; align-items: center;">
                            <c:forEach begin="${1}" end="${requestScope.numRA}" var="i">
                                <p class="dispage" style="background-color: #000;  width: 20px; height: 30px; display: block; margin-right: 5px; cursor: pointer; clear: both"><a class="${i==pageRA?'active':''}" style="color: white; padding-left: 0px; justify-content: center;align-items: center;" href="listrequest?pageRA=${i}">${i}</a></p>                                
                                </c:forEach>
                        </div>
                    </div>
                </section>
                    ...
                    <div class="pagination-container">
                        <div style="display: flex; justify-content: center; align-items: center;">
                            <c:forEach begin="${1}" end="${requestScope.numRR}" var="i">
                                <p class="dispage" style="background-color: #000;  width: 20px; height: 30px; display: block; margin-right: 5px; cursor: pointer; clear: both"><a class="${i==pageRR?'active':''}" style="color: white; padding-left: 0px; justify-content: center;align-items: center;" href="listrequest?pageRR=${i}">${i}</a></p>                                
                                </c:forEach>
                        </div>
                    </div>
                </section>
                <section id="cancel" class="cancel">
                    <h3 class="title">Cancel</h3>
                    <table id="myTable" class="table table-striped">
                        <thead>
                            <tr>
                                <th>Mentor</th>
                        ...
                        <tbody>
                            <c:set var="pageRC" value="${requestScope.pageRC}"/>
                            <c:forEach items="${requestScope.dataRC}" var="s" varStatus="loop">
                                <tr>
                                    <td>${s.mentor.fullname}</td>
                                    <td>${s.skill}</td>
                                    <td>${s.title}</td>
                    ...
                    <div class="pagination-container">
                        <div style="display: flex; justify-content: center; align-items: center;">
                            <c:forEach begin="${1}" end="${requestScope.numRC}" var="i">
                                <p class="dispage" style="background-color: #000;  width: 20px; height: 30px; display: block; margin-right: 5px; cursor: pointer; clear: both"><a class="${i==pageRC?'active':''}" style="color: white; padding-left: 0px; justify-content: center;align-items: center;" href="listrequest?pageRC=${i}">${i}</a></p>                                
                                </c:forEach>
                        </div>
                    </div>
                </section>
                        ...
                        <tbody>
                            ...
                            <c:forEach items="${requestScope.dataRPa}" var="s" varStatus="loop">
                                <tr>
                                    ...
                                    <td>${s.timeSchedule.year}</td>
                                    <td>${s.timeSchedule.day}-${s.timeSchedule.timeSlot.time}</td>
                                    <c:if test="${loop.index == 0 || !s.mentor.fullname.equals(requestScope.dataRPa[loop.index-1].mentor.fullname) ||
                                                  !s.title.equals(requestScope.dataRPa[loop.index-1].title) ||
                                                  !s.content.equals(requestScope.dataRPa[loop.index-1].content) ||
                                                  s.startWeek != requestScope.dataRPa[loop.index-1].startWeek ||
                                                  s.endWeek != requestScope.dataRPa[loop.index-1].endWeek ||
                                                  s.timeSchedule.year != requestScope.dataRPa[loop.index-1].timeSchedule.year }">
                                          <td>
                                              <form id="confirmForm-${s.idRequest}" action="listrequest" method="post" onsubmit="return confirmCompletion(this);">
                                                  <input type="hidden" name="idr" value="${s.idRequest}"/>
                                                  <input type="hidden" name="statusId" value="12"/>
                    ...
                    <div class="pagination-container">
                        <div style="display: flex; justify-content: center; align-items: center;">
                            <c:forEach begin="${1}" end="${requestScope.numRPa}" var="i">
                                <p class="dispage" style="background-color: #000;  width: 20px; height: 30px; display: block; margin-right: 5px; cursor: pointer; clear: both"><a class="${i==pageRPa?'active':''}" style="color: white; padding-left: 0px; justify-content: center;align-items: center;" href="listrequest?pageRPa=${i}">${i}</a></p>                                
                                </c:forEach>
                        </div>
                    </div>
                </section>
    ...
    <script>
        ...
        let getCancel = document.getElementById("getCancel");
        ...
        function removeClass() {
            ...
            if (getCancel)
                getCancel.classList.remove('selected');
            if (getPaid)
        ...
        if (getCancel) {
            getCancel.addEventListener('click', function (e) {
                if (window.innerWidth > 1040) {
                    e.preventDefault();
                    removeClass();
                    cancel.classList.add('view');
                    getCancel.classList.add('selected');
                }
            });
        }
